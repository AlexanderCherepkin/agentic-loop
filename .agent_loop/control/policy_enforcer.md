...
1. **Load policies** — fetch `active_policies`; if any missing, substitute with default conservative rule.
...
5. **Apply resolution** — `most_restrictive` chooses deny over allow; `hierarchy` uses policy priority order; `timestamp` prefers newer rule; `human_override` checks for approved exception ticket.
