...
**Цель:** Создать расширяемую систему оркестрации из специализированных агентов/файлов, реализующую цикл ReAct (Reasoning + Acting) с многоуровневой защитой, взаимной проверкой и самокоррекцией.  
...
**Масштаб:** агенты/файлы, несколько слоёв, категории инструментов + Figma-to-code MCP сервер + Backend Spec Bridge MCP сервер.
...
### 2.3 Безопасность (safety-control)
...
### 2.4 Взаимная проверка (mutual_check)
...
### 2.6 Подагенты цикла ReAct (tooll_subagents)
Цикл разбит на фазы:
1. **user**: request — парсинг и классификация; context — сбор контекста; limitations — каталог ограничений; design_intake — распознавание дизайн-проектов (Figma URL, node ID, локальный JSON, дизайн-бриф) и формирование `design_descriptor`; client_brief_agent — структурированный PM-бриф (цели, аудитория, CTA, референсы, стек, ограничения).
2. **planning**: task_scoping_agent — классификация задачи и выбор режима автоматизации; spec_approval_gate — интервью, написание спецификации и получение явного одобрения; verification_planner — вывод плана верификации из одобренной спецификации; task_decomposition — декомпозиция; cost_risk_assessment — оценка стоимости и риска; tool_plan_selection — выбор инструментов; internal_monologue — явное рассуждение; figma_design_analyst — анализ дизайна и генерация структуры/спецификации/кода через MCP; figma_precise_mode_auditor — Builder.io-style readiness audit перед генерацией кода; design_to_code_planner — принятие решения о выдаче технического задания или готового кода; premium_design_analyst — формирование премиального визуального направления, типографического стека и ожидания пользователя; premium_design_system_generator — генерация `DESIGN.md` и `design_tokens.json` на основе выбранного премиального направления; copywriting_agent — генерация landing-page текста (headline, CTA, hero, alt, meta) на основе брифа; estimation_proposal_agent — оценка сроков/стоимости и формирование SOW/предложения для клиента; project_starter_agent — выбор и материализация стартового шаблона (landing/SaaS/portfolio/e-commerce) с файлами, README и командами; backend_spec_bridge — сопоставление backend-спецификации с UI и генерация backend-слоя; responsive_composer — генерация breakpoint-вариантов и constraint-классов Tailwind; component_registry — построение реестра Figma-компонентов (Component Sets, Variants, Instances) и генерация `src/components/ui/*.tsx`; component_mapper — сопоставление Component Sets с React-компонентами и запись mapper-файлов; asset_agent — обнаружение, классификация и планирование загрузки Figma-ассетов; image_enrichment_agent — поиск/загрузка fallback-изображений для card/hero placeholders; i18n_requirements_analyst — извлечение требований мультиязычности; i18n_language_detector — определение языка Figma-текстов; i18n_key_extractor — преобразование текста в i18n-ключи; i18n_dictionary_generator — генерация словарей для целевых локалей; i18n_routing_planner — планирование Next.js i18n routing; i18n_component_rewriter — манифест замены литералов на `t()`; i18n_optimizer — оптимизация загрузки словарей; analytics_requirements_analyst — извлечение требований аналитики; analytics_provider_selector — выбор провайдеров аналитики; analytics_event_mapper — преобразование Figma-взаимодействий в события; analytics_script_injector — планирование безопасной вставки скриптов; analytics_optimizer — минимизация влияния аналитики; cookie_consent_jurisdiction_mapper — маппинг юрисдикций согласия; cookie_consent_policy_generator — генерация локализованной политики cookies; cookie_consent_banner_planner — проектирование UI баннера согласия; auth_requirements_analyst — извлечение требований идентификации/аутентификации; auth_provider_selector — выбор провайдера идентификации (Clerk/Auth0); cms_requirements_analyst — извлечение требований динамических секций (блог, портфолио, кейсы); cms_source_selector — выбор источника данных (local_markdown, Notion, Contentful, Strapi, Prisma, Airtable, Google Sheets, CMS API); accessibility_requirements_analyst — извлечение требований доступности по WCAG 2.1; accessibility_checker_planner — планирование статических и браузерных проверок доступности; pwa_requirements_analyst — извлечение требований PWA и performance budget; pwa_optimizer — выбор стратегии манифеста, service worker, offline-страницы и оптимизации ресурсов; design_token_docs_requirements_analyst — извлечение требований к документации токенов для клиента/команды; design_token_docs_format_selector — выбор форматов документации (markdown/json/html) и плана выдачи; multi_page_planner — планирование многостраничной маршрутизации Next.js App Router; storybook_planner — планирование генерации Storybook stories и конфигурации; deploy_planner — планирование публикации сгенерированного сайта; preview_planner — планирование preview/approval workflow для согласования с клиентом.
3. **execution**: tool_invocation — диспетчеризация; safety_guardrails — live runtime safety; human_approval — тактическое одобрение; action_logging — неизменяемый журнал выполнения; i18n_runtime_integrator — материализация next-intl артефактов; i18n_fallback_resolver — разрешение недостающих переводов; analytics_runtime_integrator — материализация аналитики и баннера согласия; cookie_consent_blocker — блокировка скриптов до согласия; auth_runtime_integrator — материализация Clerk/Auth0 обёрток, middleware и env-шаблона; cms_runtime_integrator — материализация слоя данных Next.js App Router для динамических секций; accessibility_runtime_integrator — запуск статического WCAG-аудита через `runtime/accessibility/AccessibilityEngine`; pwa_runtime_integrator — материализация манифеста, service worker, offline-страницы и диагностики бюджетов через `runtime/pwa/PwaEngine`; design_token_docs_runtime_integrator — генерация markdown/json/html документации по токенам через `runtime/design_token_docs/DesignTokenDocsEngine`; multi_page_runtime_integrator — материализация Next.js App Router страниц, Navigation.tsx, sitemap.ts и robots.ts через `runtime/multi_page/MultiPageEngine`; storybook_runtime_integrator — генерация .stories.tsx и конфигурации Storybook через `runtime/storybook/StorybookEngine`; deploy_runtime_integrator — запуск Vercel/Netlify/generic deploy через `runtime/deploy/DeployEngine` (dry-run по умолчанию); preview_runtime_integrator — запуск preview workflow (dev server, screenshot, QR, feedback) через `runtime/preview/PreviewEngine`; git_publish_runtime_integrator — публикация сгенерированного репозитория на GitHub/GitLab; notification_runtime_integrator — отправка уведомлений о завершении pipeline; project_developer — генерация кода по архитектурному манифесту.
4. **observability**: environment_result — снимок среды; runtime_output — парсинг stdout/stderr/exit codes; file_context — отслеживание мутаций файлов; memory_enrichment — обогащение долгосрочной памяти; headroom_compressor/headroom_retriever — сжатие/восстановление больших артефактов; memanto_remember/recall/answer — семантическая память; mem0_remember/recall/list — гибридная долгосрочная память; i18n_audit_agent — аудит мультиязычности; analytics_audit_agent — аудит аналитики и согласия; auth_audit_agent — аудит реализации идентификации/аутентификации; cms_audit_agent — аудит реализации динамических секций и fallback; accessibility_audit_agent — аудит реализации доступности и WCAG-соответствия; pwa_audit_agent — аудит реализации PWA, performance budget и offline-UX; design_token_docs_audit_agent — аудит документации токенов: полнота, корректность, соответствие аудитории; multi_page_audit_agent — аудит многостраничной маршрутизации и SEO-артефактов; storybook_audit_agent — аудит Storybook stories и конфигурации; deploy_audit_agent — аудит результата публикации и наличия deploy URL; preview_audit_agent — аудит preview workflow: наличие скриншота, QR, feedback и approval статуса.
5. **self_correction**: result_validation — проверка результатов; plan_adjustment — коррекция плана; recursion_or_termination — решение продолжать/завершить; assistance_request — эскалация к человеку; goal_evaluator — fast-critic оценки прогресса по цели (/goal); ponytail_review — проверка over-engineering для сгенерированного/отрефакторенного кода; regression_guard — сравнение текущих артефактов (скриншот, layout, Lighthouse, файлы) с предыдущей итерацией и обнаружение деградации; anti_slop_validator — hard-gate проверка премиального дизайна на банальные паттерны и запрещённые шрифты; i18n_rtl_validator — проверка RTL-поддержки; i18n_missing_key_guard — проверка полноты ключей; analytics_privacy_validator — проверка приватности аналитики; auth_validator — проверка корректности обёрток идентификации/аутентификации; cms_validator — проверка корректности интеграции динамических секций; accessibility_validator — проверка результатов WCAG-аудита и формирование refinement_actions; pwa_validator — проверка результатов PWA/performance audit и формирование refinement_actions; design_token_docs_validator — проверка полноты документации токенов и формирование refinement_actions; multi_page_validator — проверка корректности многостраничной маршрутизации и отсутствия дублирующихся slug; storybook_validator — проверка наличия stories для ключевых компонентов и валидности конфигурации; deploy_validator — проверка результата deploy и доступности deploy URL; preview_validator — проверка результатов preview workflow и формирование refinement_hints при отказе; code_review_validator — проверка кода и применение патча; diff_patch_applier — применение diff-патча; security_scan_validator — проверка безопасности; quality_evaluator_agent — оценка качества артефактов; cost_audit_agent — аудит стоимости.
6. **result**: solution — финальное решение; modified_files — инвентарь изменений; action_report — отчёт о действиях; summary_recommendations — рекомендации; skill_packager — материализация переиспользуемого skill из gotcha-извлечений после явного одобрения.

### 2.7 Инструментальные агенты (tools_*)
Категории инструментальных агентов с cross-cutting optimizer на категорию. Кроме того, `mcp_servers/figma_server.py` предоставляет MCP-обёртку вокруг `figma-agent-core/` для приёма дизайн-проектов:
...
- **tools_runcom** — run_command pipeline (sandboxed): command_builder, sandbox_agent, env_manager, timeout_watcher, executor_agent, output_collector, error_analyzer, write_executor, write_planner, command_optimizer.
...
- **tools_browser** — headless_automation pipeline (browser-lifecycle): session_manager, navigation_engine, screenshot_agent, dom_extractor, selector_resolver, interaction_agent, network_interceptor, cookie_storage_agent, captcha_challenge_agent, error_handler, visual_qa_agent, browser_optimizer.
...
Каждая категория `tools_*` — независимый pipeline агентов с оптимизатором. Pipeline типы: linear (read), diamond (search), safety-gated (replace), sandboxed (runcom), framework-dispatch (runtest), session-stateful (terminal), analysis-planning (manangr), query-lifecycle (database), request-lifecycle (web), store-lifecycle (memory), headless-automation (browser).
...
### 6.9 Многостраничная маршрутизация (multi-page routing)
- Для клиентских сайтов с несколькими экранами система должна генерировать многостраничный Next.js App Router через `runtime/multi_page/MultiPageEngine`.
- Генерируемые артефакты: `app/[slug]/page.tsx` для каждой страницы, `app/page.tsx` для home, `src/app/components/Navigation.tsx`, `app/sitemap.ts`, `app/robots.ts`.
- Режим `--routing-artifacts` позволяет сгенерировать только навигацию/SEO-артефакты без перезаписи существующих страниц.
- Планирование: `multi_page_planner.md`; исполнение: `multi_page_runtime_integrator.md`; самокоррекция: `multi_page_validator.md`; аудит: `multi_page_audit_agent.md`.

### 6.10 Storybook интеграция
- Для UI-компонентов сгенерированного фронтенда система должна генерировать Storybook stories и конфигурацию через `runtime/storybook/StorybookEngine`.
- Генерируемые артефакты: `.stories.tsx` для компонентов в `src/components/ui` и `src/app/components`, `.storybook/main.ts`, `.storybook/preview.ts`, а также патч `package.json` со скриптами `storybook` и `build-storybook`.
- Планирование: `storybook_planner.md`; исполнение: `storybook_runtime_integrator.md`; самокоррекция: `storybook_validator.md`; аудит: `storybook_audit_agent.md`.

### 6.11 Публикация (deploy execution)
- Для готового сайта система должна запускать публикацию на Vercel/Netlify/generic через `runtime/deploy/DeployEngine`.
- По умолчанию работает в режиме dry-run; реальная публикация включается флагом `--deploy-live`.
- Поддерживаемые провайдеры: `vercel` (`npx vercel --prod --yes`), `netlify` (build + `npx netlify deploy --prod`), `generic` (build + инструкция по загрузке).
- Движок извлекает deploy URL из stdout/stderr и возвращает его в отчёте.
- Планирование: `deploy_planner.md`; исполнение: `deploy_runtime_integrator.md`; самокоррекция: `deploy_validator.md`; аудит: `deploy_audit_agent.md`.

### 6.12 Preview / approval workflow
- Перед финальной публикацией система должна предоставлять клиенту preview: dev server, скриншот страницы, QR-код и форму feedback через `runtime/preview/PreviewEngine`.
- `PreviewEngine` оборачивает `figma-agent-core/preview_workflow.py` и передаёт параметры site_dir, page_url, viewport, dev_command, feedback_file.
- Результат включает статус `approved`/`rejected`, причины отказа и `refinement_hints` для следующей итерации.
- Планирование: `preview_planner.md`; исполнение: `preview_runtime_integrator.md`; самокоррекция: `preview_validator.md`; аудит: `preview_audit_agent.md`.
...
- Отсутствие битых ссылок (исключая документационные цели: README, API, CHANGELOG, MEMORY).
- Отсутствие изолированных агентов.
- Проверка осуществляется скриптом `validate_cross_references.js`.

### 7.2 Полнота шаблона
- Все агенты содержат все обязательные разделы Algorithmic template.
- Отсутствие заглушек (stub'ов).
...
- [x] Все агенты/файлы реализованы по Algorithmic template.
- [x] Отсутствие заглушек (stub'ов).
- [x] Все агенты/файлы связаны в единый граф ссылок (отсутствие изолированных).
- [x] Отсутствие битых ссылок (после фильтрации документационных целей).
- [x] Трёхконтурная безопасность соблюдена: safety-control → mutual_check → control.
- [x] ReAct цикл декомпозирован на подагентов в фазах с условными переходами (Conditional Edges).
- [x] Категории инструментов (tools_*) реализованы с pipeline-специфичной архитектурой и оптимизаторами.
...
- [x] Скрипт консистентности (`validate_consistency.js`) показывает отсутствие errors и warnings.
...
- Все агенты/файлы реализованы.
- Отсутствие stubs.
- Отсутствие изолированных агентов.
- Отсутствие битых ссылок (валидатор `validate_cross_references.js` пройден).
- Отсутствие ошибок консистентности (валидатор `validate_consistency.js` пройден без errors и warnings).
...
- Добавлены модули завершения клиентского сайта: multi-page routing (`runtime/multi_page/MultiPageEngine`), Storybook integration (`runtime/storybook/StorybookEngine`), deploy execution (`runtime/deploy/DeployEngine` с dry-run по умолчанию) и preview/approval workflow (`runtime/preview/PreviewEngine`), каждый с агентами планирования, исполнения, самокоррекции и аудита; интегрированы в Figma conductor как `--multi-page`, `--routing-artifacts`, `--storybook`, `--deploy`, `--deploy-live`.
...
- CI `.github/workflows/ci.yml` запускает tier-ы pytest: `core`, `mcp` и `tests/integration/`; figma-tier исключён из обязательного CI, так как требует `FIGMA_TOKEN`/`FIGMA_URL`. Также CI запускает `ruff check`, `black --check`, `mypy`, и `pytest --cov` с порогом покрытия, настраиваемым в `pyproject.toml`.
