...
- `design_descriptor`: structured object present only when `request_type=design_project`:
  ...
  - `metadata`: `{ title, detected_language, has_assets, has_components }`
...
2. **Detect design signals** — check for:
   ...
   - Keywords: "макет", "дизайн", "Figma", "section", "frame", "компонент", "верстка", "React по макету".
...
7. **Build fallback for non-design requests** — if no design signals, return `request_type` from conventional `request.md` classification and leave `design_descriptor=null`.
