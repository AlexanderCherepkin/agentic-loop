...
`task_scoping_agent.md`, `spec_approval_gate.md`, `spec_lock.md`, `spec_compliance_validator.md`, `gotcha_extractor.md`.
...
4. **Scope and spec gate (conditional)** — invoke `tooll_subagents/planning/task_scoping_agent.md` to classify the task as `trivial`, `medium`, or `large` and decide whether an approved spec is required. For `trivial` tasks, skip spec approval and proceed directly to a single lightweight action. For `medium` and `large` tasks, invoke `tooll_subagents/planning/spec_approval_gate.md` to conduct the interview, produce a written spec, and require explicit user approval. If `spec_status` is not `approved`, short-circuit to Result synthesis with the clarifying questions or the pending spec and `termination_status=success`. If the user approves, store `approved_spec` in session state and continue to the Plan phase.
...
9. **Extract gotchas** — if the session produced corrections, iterations, or user feedback, invoke `tooll_subagents/observability/gotcha_extractor.md` to capture reusable pitfalls and propose a skill candidate. Do not create a skill without explicit user approval.
