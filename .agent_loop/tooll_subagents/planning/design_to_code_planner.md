...
8. **Merge copywriting package** — if `design_blueprint.metadata.copy_package` exists from `copywriting_agent.md`, include it in the handoff package and surface it to `execution` agents that render text. If the blueprint has no Figma source (`design_source=design_brief`), ensure the handoff package carries the `copy_package` as the main content artifact.
