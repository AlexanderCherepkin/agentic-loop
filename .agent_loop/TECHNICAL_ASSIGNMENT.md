...
- CI `.github/workflows/ci.yml` запускает tier-ы pytest: `core`, `mcp` и `tests/integration/`; figma-tier исключён из обязательного CI, так как требует `FIGMA_TOKEN`/`FIGMA_URL`.
