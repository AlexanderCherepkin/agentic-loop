...
13b. **Extract i18n requirements** — invoke `i18n_requirements_analyst.md` (`tooll_subagents/planning/i18n_requirements_analyst.md`) to determine target locales, default locale, RTL needs, and translation scope from the request and design brief.
13c. **Detect source language** — if i18n is enabled, invoke `i18n_language_detector.md` (`tooll_subagents/planning/i18n_language_detector.md`) on the cached Figma export to produce a language profile and identify the primary source locale.
13d. **Extract i18n keys** — invoke `i18n_key_extractor.md` (`tooll_subagents/planning/i18n_key_extractor.md`) to turn Figma text nodes into stable namespaced keys and build a `key_registry` for downstream translation.
13e. **Generate dictionaries** — if `translation_scope` is `full` or `ui_only`, invoke `i18n_dictionary_generator.md` (`tooll_subagents/planning/i18n_dictionary_generator.md`) to produce `messages/*.json` dictionaries for every target locale, preserving interpolations and fallback chains.
13f. **Plan i18n routing and rewriting** — invoke `i18n_routing_planner.md` (`tooll_subagents/planning/i18n_routing_planner.md`) and `i18n_component_rewriter.md` (`tooll_subagents/planning/i18n_component_rewriter.md`) to decide Next.js routing strategy and produce a rewrite manifest that replaces literal strings with `t('...')` calls.
13g. **Optimize i18n loading** — invoke `i18n_optimizer.md` (`tooll_subagents/planning/i18n_optimizer.md`) to choose SSG/dynamic/lazy-namespace loading strategy and estimate bundle impact.
13h. **Map analytics events** — invoke `analytics_event_mapper.md` (`tooll_subagents/planning/analytics_event_mapper.md`) to convert Figma prototype interactions and CTAs into provider-agnostic analytics event definitions attached to the blueprint.
13i. **Plan design token docs handoff** — invoke `design_token_docs_requirements_analyst.md` (`tooll_subagents/planning/design_token_docs_requirements_analyst.md`) and `design_token_docs_format_selector.md` (`tooll_subagents/planning/design_token_docs_format_selector.md`) to decide whether to produce client/team handoff docs (`docs/DESIGN_TOKENS.md`, `docs/design_tokens.docs.json`, optional `docs/design_tokens.html`) from `design_tokens.json` and `component_registry.json`; attach the docs plan to the blueprint.
13j. **Generate copywriting package** — if `client_brief` is present (from the intake phase or from a design-brief request), invoke `copywriting_agent.md` (`tooll_subagents/planning/copywriting_agent.md`) to produce a `copy_package` (headline, hero text, CTAs, alt texts, meta tags, section copy). Merge it into `design_blueprint.metadata.copy_package` and route the package into `i18n_key_extractor.md` so all generated copy receives stable keys and dictionary entries. When `design_source=design_brief` and no Figma source exists, the `copy_package` becomes the primary content artifact for downstream page composition.
13k. **Generate estimation and proposal package** — if `client_brief` is present and either the request explicitly asks for an estimate/proposal/SOW or `client_brief.limits` contains budget/deadline hints, invoke `estimation_proposal_agent.md` (`tooll_subagents/planning/estimation_proposal_agent.md`) with the brief, blueprint, and copy package. Merge the returned `proposal_package` into `design_blueprint.metadata.proposal_package`. If the request's primary intent is a proposal (`output_mode` includes `proposal` or explicit ask), set `design_blueprint.next_phase_hint=result` so the SOW is surfaced to the user immediately.
13l. **Generate project starter package** — if `client_brief` is present and the request is a client order, invoke `project_starter_agent.md` (`tooll_subagents/planning/project_starter_agent.md`) with the brief, blueprint, copy package, and proposal package. Merge the returned `starter_package` into `design_blueprint.metadata.starter_package`. If the request explicitly asks for a starter/scaffold/bootstrap (`output_mode` includes `starter` or explicit ask), set `design_blueprint.next_phase_hint=execution` so the starter files are written by the execution layer.
...
| i18n requirements conflict with design source | Log conflict to `audit_logger.md`; keep design source locale as default and flag for `i18n_requirements_analyst.md` review |
| i18n language detector returns low confidence | Include profile in `diagnostics`; route to `i18n_key_extractor.md` with `translation_scope=keys_only` warning |
| i18n key extraction produces no keys | Set `translation_scope=none`; continue pipeline; log to `audit_logger.md` |
| i18n dictionary generation fails | Set `status=partial`; continue with source-language only; log to `audit_logger.md` |
| Estimation input missing or incomplete | Attach low-confidence `proposal_package` with `missing_inputs`; do not block pipeline |
| Proposal mode conflicts with code generation | Honor explicit `output_mode`; if proposal is primary, return blueprint with `next_phase_hint=result` |
| Starter template cannot be inferred | Default to `landing`; flag `missing_inputs` |
| Starter scaffolding conflicts with existing files | Attach `safe_overwrite_warning`; do not force writes from planning layer |
