...
4. **Iterate invocations** — for each tool in `tool_plan`:
   ...
   d1. **i18n tools** — if the tool name starts with `i18n_` or is listed in the `i18n` category (`i18n_language_detector`, `i18n_key_extractor`, `i18n_dictionary_generator`, `i18n_routing_planner`, `i18n_component_rewriter`, `i18n_optimizer`, `i18n_runtime_integrator`, `i18n_fallback_resolver`), marshal parameters and submit via `mcp_servers/gateway.py` or `orchestrator/dispatcher.md` to the planning/execution agents; if `mcp_gateway` reports degraded, fall back to `runtime/i18n/engine.py` directly.
   d2. **Analytics and consent tools** — if the tool name starts with `analytics_` or `cookie_consent_` or is listed in the `analytics` category (`analytics_requirements_analyst`, `analytics_provider_selector`, `analytics_event_mapper`, `analytics_script_injector`, `analytics_optimizer`, `analytics_runtime_integrator`, `cookie_consent_jurisdiction_mapper`, `cookie_consent_policy_generator`, `cookie_consent_banner_planner`, `cookie_consent_blocker`), marshal parameters and submit via `mcp_servers/gateway.py` or `orchestrator/dispatcher.md`; if `mcp_gateway` reports degraded, fall back to `runtime/analytics/engine.py` directly.
   d3. **Design token docs tools** — if the tool name starts with `design_token_docs_` or is listed in the `design_token_docs` category (`design_token_docs_requirements_analyst`, `design_token_docs_format_selector`, `design_token_docs_runtime_integrator`, `design_token_docs_validator`, `design_token_docs_audit_agent`), marshal parameters and submit via `mcp_servers/gateway.py` or `orchestrator/dispatcher.md`; if `mcp_gateway` reports degraded, fall back to `runtime/design_token_docs/engine.py` directly.
...
| i18n MCP tool unavailable | Degrade to `runtime/i18n/engine.py` direct invocation; log degradation to `audit_logger.md` |
| Analytics MCP tool unavailable | Degrade to `runtime/analytics/engine.py` direct invocation; log degradation to `audit_logger.md` |
| Cookie consent MCP tool unavailable | Degrade to `runtime/analytics/engine.py` direct invocation and disable provider loading; log to `audit_logger.md` |
